...
void EMPGrenadeExplode(  )
{
	...
	for ( te = world; te = findradius( te, self->s.v.origin, 240 ); )
	{
		...
		if ( streq( te->s.v.classname, "building_dispenser" )
		     || streq( te->s.v.classname, "building_sentrygun" ) )
		{
			if ( !
		...
		if ( streq( te->s.v.classname, "player" ) || te->s.v.touch == ( func_t ) BackpackTouch )
		{
			       ...
			       && te->team_no == PROG_TO_EDICT( self->s.v.owner )->team_no ) )
			{
				...
				if ( expsize > 0 )
				{
					...
					{
						TF_T_Damage( te, self, PROG_TO_EDICT( self->s.v.owner ), expsize, 2,
							     4 );
...
/*void Eng_SGUp()
{
 ...
 while (sg->s.v.weapon == 3 && sg->s.v.ammo_shells == 144 && sg->s.v.ammo_rockets == 20 && sg->s.v.health == sg.max_health) {
  if (sg == world) {
...
void Eng_DispLoad()
{
 ...
 self.hook_out = 0;
 power = rint(25 + disp->s.v.ammo_rockets * 1.5 + disp->s.v.ammo_cells);
 if (power > 250) 
  power = 250;
 st = ftos(power);
...
void Eng_SGReload()
{
 ...
 sg = find(world, classname, "building_sentrygun");
 while (sg->s.v.ammo_shells == sg.maxammo_shells && sg->s.v.ammo_rockets == sg.maxammo_rockets && sg->s.v.health == sg.max_health) {
  if (sg == world) {
