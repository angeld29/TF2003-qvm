...
void W_FireAxe() {
  ...
  if (trace_ent->s.v.takedamage) {
    ...
    SpawnBlood(org, 20);
    if (self->playerclass != PC_SPY || strneq(PROG_TO_EDICT(g_globalvars.trace_ent)->s.v.classname, "player")) {
      ...
      if (streq(trace_ent->s.v.classname, "building_sentrygun")) {
        ...
        if (streq(trace_ent->s.v.classname, "building_sentrygun_base")) {
          ...
          if (streq(trace_ent->s.v.classname, "player")) {
            if ((TeamFortress_isTeamsAllied(trace_ent->team_no, self->team_no) && self->team_no && teamplay) || coop) {
              healam = WEAP_SPANNER_REPAIR;
              if (self->s.v.ammo_cells < healam)
                healam = self->s.v.ammo_cells;
              if (!trace_ent->s.v.armortype)
                return;
              if (trace_ent->maxarmor - trace_ent->s.v.armorvalue < healam * 4)
                healam = ceil((trace_ent->maxarmor - trace_ent->s.v.armorvalue) / 4);
              if (healam > 0) {
    ...
    if (streq(trace_ent->s.v.classname, "player")) {
      if ((TeamFortress_isTeamsAllied(trace_ent->team_no, self->team_no) && self->team_no) || coop) {
        ...
        if (healam > 0 && trace_ent->s.v.health < trace_ent->s.v.max_health) {
          ...
          if (trace_ent->s.v.health >= trace_ent->s.v.max_health && trace_ent->s.v.health < trace_ent->s.v.max_health + WEAP_MEDIKIT_OVERHEAL) {
            healam = 5;
            ...
            if (healam > 0) {
              ...
              if (!((int)trace_ent->s.v.items & IT_SUPERHEALTH)) {
                trace_ent->s.v.items = (int)trace_ent->s.v.items | IT_SUPERHEALTH;
                newmis = spawn();
...
================
*/
void SpawnBlood(vec3_t org, float damage) { TempEffectCount(org, TE_BLOOD, 1); }
...
void FireSniperBullet(vec3_t direction, float damage) {
  ...
  src[2] = self->s.v.absmin[2] + self->s.v.size[2] * 0.7;
  ClearMultiDamage();
  traceline(PASSVEC3(src), src[0] + direction[0] * 4096, src[1] + direction[1] * 4096, src[2] + direction[2] * 4096, false, self);
...
void W_FireSniperRifle() {
  ...
  if (tfset(snip_range_fix))
    traceline(PASSVEC3(src), src[0] + dir[0] * 8092, src[1] + dir[1] * 8092, src[2] + dir[2] * 8092, false, self);
  else
    traceline(PASSVEC3(src), src[0] + dir[0] * 3072, src[1] + dir[1] * 3072, src[2] + dir[2] * 3072, false, self);
    ...
    if (streq(trace_ent->s.v.classname, "player")) {
      ...
      if (zdif < 0) {
        ...
        if (zdif > 20) {
          ...
          VectorSubtract(trace_ent->s.v.origin, self->s.v.origin, trace_ent->head_shot_vector);
...
void W_FireAssaultCannon() {
  ...
  if (self->heat < ((ASSAULT_SPREAD_TIME / 0.2) * 0.1)) {
    VectorScale(spread, (1 + (self->heat / ((ASSAULT_SPREAD_TIME / 0.2) * 0.1)) * (ASSAULT_SPREAD_MULTIPLY - 1)), spread);
...
void BecomeExplosion() { dremove(self); }
...
void W_FireRocket() {
  ...
  setorigin(newmis, self->s.v.origin[0] + g_globalvars.v_forward[0] * 8, self->s.v.origin[1] + g_globalvars.v_forward[1] * 8, self->s.v.origin[2] + g_globalvars.v_forward[2] * 8 + 16);
}
...
void LightningDamage(vec3_t p1, vec3_t p2, gedict_t *from, float damage) {
  ...
  traceline(p1[0] - f[0], p1[1] - f[1], p1[2] - f[2], p2[0] - f[0], p2[1] - f[1], p2[2] - f[2], false, self);
  if (PROG_TO_EDICT(g_globalvars.trace_ent) != e1 && PROG_TO_EDICT(g_globalvars.trace_ent) != e2 && PROG_TO_EDICT(g_globalvars.trace_ent)->s.v.takedamage) {
    LightningHit(from, damage);
  }
}
...
void W_FireLightning() {
  ...
  org[2] += 16;
  traceline(PASSVEC3(org), org[0] + g_globalvars.v_forward[0] * 600, org[1] + g_globalvars.v_forward[1] * 600, org[2] + g_globalvars.v_forward[2] * 600, true, self);
  trap_WriteByte(MSG_MULTICAST, SVC_TEMPENTITY);
...
void W_FireGrenade() {
  ...
  if (self->s.v.v_angle[0]) {
    newmis->s.v.velocity[0] = g_globalvars.v_forward[0] * 600 + g_globalvars.v_up[0] * 200 + crandom() * g_globalvars.v_right[0] * 10 + crandom() * g_globalvars.v_up[0] * 10;
    newmis->s.v.velocity[1] = g_globalvars.v_forward[1] * 600 + g_globalvars.v_up[1] * 200 + crandom() * g_globalvars.v_right[1] * 10 + crandom() * g_globalvars.v_up[1] * 10;
    newmis->s.v.velocity[2] = g_globalvars.v_forward[2] * 600 + g_globalvars.v_up[2] * 200 + crandom() * g_globalvars.v_right[2] * 10 + crandom() * g_globalvars.v_up[0] * 10;
  } else {
...
int W_BestWeapon() {
  ...
  do {
    ...
    if (wi->w == WEAP_ASSAULT_CANNON && (self->s.v.ammo_cells >= 6 && self->s.v.ammo_shells > self->assault_min_shells)) {
      return WEAP_ASSAULT_CANNON;
    }
    if (wi->ammo_shells && self->s.v.ammo_shells < wi->ammo_shells)
      continue;
    if (wi->ammo_nails && self->s.v.ammo_nails < wi->ammo_nails)
      continue;
    if (wi->ammo_rockets && self->s.v.ammo_rockets < wi->ammo_rockets)
      continue;
    if (wi->ammo_cells && self->s.v.ammo_cells < wi->ammo_cells)
      continue;
    return *w;
  } while (*(++w));
  return 0;
...
// 0 ok
{
  if (!(weapon & self->weapons_carried))
    return 3;
  switch (weapon) {
  case WEAP_HOOK:
    if (!tfset(allow_hook))
    ...
    break;
  case WEAP_SNIPER_RIFLE:
    if (self->s.v.ammo_shells < tfset_snip_ammo)
      return 1;
    break;
...
void W_ChangeWeapon() {
  ...
  if (wi->impulse == 7 && fl == WEAP_GRENADE_LAUNCHER)
    wm = 1;
...
const struct weapon_s weapon_cycle[] = {{WEAP_AXE, 0},
                                        {WEAP_SPANNER, 0},
                                        {WEAP_SHOTGUN, 0},
                                        {WEAP_LASER, 0},
                                        {WEAP_TRANQ, 0},
...
void W_WeaponFrame() {
  ...
  if (self->current_menu > 0) {
    Player_Menu();
    if (self->s.v.impulse > 0 && (self->current_menu == 23 || self->current_menu == 24 || self->current_menu == 25))
