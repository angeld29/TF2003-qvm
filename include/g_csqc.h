...
//===========================================================================
// Контракт использования svc 83 (cgamepacket) — обязателен:
//   - писать 83 ТОЛЬКО в MSG_MULTICAST и первым байтом пустого буфера;
//   - каждая отправка svc — отдельной функцией: сборка пакета и trap_multicast()
//     в одной функции, без досрочных return/ветвлений между первой записью и flush;
//   - не смешивать svc в одном буфере (обычный svc и cgamepacket — разными вызовами);
//   - to ∈ MULTICAST_PVS/PHS/ALL (+_R), не MULTICAST_MVD_HIDDEN;
//   - не писать 83 в MSG_ONE/MSG_ALL/MSG_BROADCAST/MSG_INIT;
//   - не полагаться на содержимое буфера между вызовами.
// Движок рассылает пакет только (FTE_PEXT_CSQC && csqcactive) клиентам и
// рекордеру при sv_mvd_csqc; смешанный/незафлушенный буфер он не разбирает
// (нет таблицы длин svc), поэтому правила выше обязательны.
//===========================================================================
